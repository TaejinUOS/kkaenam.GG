...
- 왼쪽에 `깨남.GG`, 중앙에 `상대법 / 전적 / 통계 / 티어표 / 강의`, 오른쪽에 마이페이지와 프로필
...
- 상단 카피 `누굴 상대해?` 아래에 `탑 / 정글 / 미드 / 원딜 / 서폿`을 종이 인덱스 탭처럼 배치하고 현재 포지션은 Acid 면으로 표시한다.
...
| 헤더 메뉴 구성 | `상대법 / 전적 / 통계 / 티어표 / 강의` + 마이페이지. `상대법`을 뺀 전부가 공통 `추후 개발` 프레임으로 간다 | 6.1의 메뉴 구성과 PRD 5.6의 안내 화면 요구를 합친 결과다 |
| 영상 탭 | 임시 썸네일을 만들지 않고 빈 상태만 제공한다. 6.4의 2열 골격은 자리만 잡아 둔다 | PRD 3.2가 가짜 데이터를 금지하고, 영상 출처·검수 주체는 PRD 15의 미결정 8번이다 |
| 서체 | Bagel Fat One과 IBM Plex Mono는 `next/font`로, Pretendard는 배포 CDN의 동적 서브셋으로 불러온다 | Pretendard가 Google Fonts에 없다. 10장의 라이선스 확인 항목은 그대로 남는다 |
...
- Bagel Fat One과 Pretendard의 한글 조합, 특히 긴 카테고리명 `브루저/원딜`의 포스터 라벨 처리
