...
| 주석 | `본문[^쿨타임]` + `[^쿨타임]: 설명` | 표식에 마우스를 올리면 그 자리에 뜬다 |
| 문서 링크 | `[[미드/아리]]` `[[미드/아리|아리]]` | 매치업 문서로 이동. 없는 문서는 빨갛게 |
...
- **주석 정의는 한 줄이다.** 여러 줄 주석은 받지 않는다. 편집자가 외울 규칙이 적은 쪽이 낫고,
  짧은 보충 설명이라는 주석의 쓰임에도 맞는다.
- **정의가 없는 주석 참조는 손대지 않고 그대로 둔다.** 조용히 사라지면 오타를 알아채지 못한다.
- **위키링크 해석은 서버에서 한다.** 챔피언 카탈로그와 운영 분류가 모두 필요한데, 클라이언트로
  내려보내면 170명이 통째로 번들에 실린다. 문서에 실제로 적힌 링크만 미리 풀어 넘긴다.
- **분류에 없는 조합(`[[미드/가렌]]`)은 빨간 링크다.** 그 주소는 어차피 404이므로(PRD FR-07)
  링크로 만들어 보내지 않는다.
